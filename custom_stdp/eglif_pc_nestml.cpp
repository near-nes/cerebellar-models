...
void nest::register_eglif_pc_nestml(const std::string &name)
{
...
eglif_pc_nestml::eglif_pc_nestml(const eglif_pc_nestml &__n) : ExtendedPostHistoryArchivingNode(), P_(__n.P_), S_(__n.S_), B_(__n.B_, *this)
{
...
void eglif_pc_nestml::update(nest::Time const &origin, const long from, const long to)
{
  ...
  for (long lag = from; lag < to; ++lag)
  {
    ...
    if (S_.cf_buffer != 0)
    {
      set_spiketime(nest::Time::step(origin.get_steps() + lag), 0.75);
      nest::SpikeEvent se;

      //------------- make CS arrive on a different receptor in DCN (rec4 in analogue to rec2 in DCN)
      se.set_offset(0.75);
      // std::cout << "PC emits a CS " << std::endl;
      //---------------
